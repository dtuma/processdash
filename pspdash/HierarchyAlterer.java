...
public class HierarchyAlterer implements ItemListener {
    ...
    private void beginChanges() throws HierarchyAlterationException {
        ...
        dashboard.props.addItemListener(this);
    ...
    private void endChanges() {
        dashboard.props.removeItemListener(this);
        dashboard.data.finishInconsistency();
        dashboard.refreshHierarchy();
        updateNodesAndLeaves(dashboard.data, dashboard.props);
    }
    ...
    private PropertyKey doAddNode(PSPDashboard dash, String path) {
        ...
        while (tok.hasMoreTokens())
            key = maybeMakeNode(dash.props, key, tok.nextToken());
        return key;
    }

    private PropertyKey maybeMakeNode(PSPProperties props,
                                      PropertyKey parent,
                                      String childName) {
        PropertyKey child;
    ...
    public void addTemplate(String path, String templateID)
        throws HierarchyAlterationException
    {
        PSPProperties templates = dashboard.getTemplateProperties();
        ...
        try {
            ...
            dashboard.props.copyFrom (templates, templateKey, newNode);
        } finally {
            endChanges();
        }
    }
    ...
    */
    ...
    protected void createDataFile(PendingDataChange p) {
        ...
        if (p.srcFile == null)
            PropertyFrame.createEmptyFile(dataDir + p.destFile);
        else
            PropertyFrame.createDataFile(dataDir + p.destFile, p.srcFile);
     ...
     */
    static void updateNodesAndLeaves(DataRepository data,
                                     PSPProperties useProps) {
