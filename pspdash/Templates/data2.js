...
function changeNotifyElem(elem) {
    ...
    if (value != valueList[elemNum]) {
        ...
        if (checkElemEditable(elem))
            addMessage("/data/formEdit.class" +
                       "?s=" + sessionID +
                       "&c=" + changeCoupon +
                       "&f=" + elemNum +
                       "&v=" + urlEncode(value));
...
function getListenURL() {
    return "/data/formListen.class?s="+sessionID+"&c="+changeCoupon;
}
...
function getRegistrationURL() {
    var uri = urlEncode(window.location.pathname +
                        window.location.hash +
                        window.location.search);
    return '/data/formRegister.class?uri=' + uri + '&' + parameterString;
}
