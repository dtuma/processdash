...
public class FilterLocalTimeLog extends AbstractLineBasedFilter {

    @EnabledFor(RedactFilterIDs.NOTES)
    private boolean stripComments;

    @EnabledFor(RedactFilterIDs.TASK_NAMES)
    private boolean remapNames;

    private HierarchyPathMapper pathMapper;

    public FilterLocalTimeLog() {
        setFilenamePatterns("^timelog(2)?.xml$");
    }

    public String getString(String line) {
        if (stripComments)
            line = discardXmlAttr(line, "comment");
        if (remapNames)
